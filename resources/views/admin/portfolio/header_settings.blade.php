...
@section('title', 'Portfolio Header Settings')
        ...
        <div class="config-card">
            <div class="card-header bg-white border-0 py-4 px-4">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h5 class="fw-bold mb-0">Portfolio Branding</h5>
                        <p class="text-muted small mb-0">Manage the titles and subtitles for your portfolio section.</p>
                    </div>
                </div>
            </div>

            <div class="card-body p-4 p-md-5 pt-0">
                <form action="{{ route('portfolio.header.update') }}" method="POST">
                    @csrf
                    <div class="row g-4">
                        <div class="col-md-12">
                            <label class="form-label fw-bold">Portfolio Title</label>
                            <input type="text" name="title" value="{{ $header->title ?? '' }}" class="form-control" placeholder="e.g. Our Latest Works" required>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-bold">Subtitle One</label>
                            <input type="text" name="subtitle_one" value="{{ $header->subtitle_one ?? '' }}" class="form-control" placeholder="e.g. Case Studies">
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-bold">Subtitle Two</label>
                            <input type="text" name="subtitle_two" value="{{ $header->subtitle_two ?? '' }}" class="form-control" placeholder="e.g. Explore our successful projects.">
                        </div>

                        <div class="col-12 mt-4 text-end border-top pt-4">
                            <button type="submit" class="btn btn-update shadow-sm">
                                <i class="bi bi-check2-circle me-2"></i> Update Portfolio Header
                            </button>
                        </div>
                    </div>
                </form>
